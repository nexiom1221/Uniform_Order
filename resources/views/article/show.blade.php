		...
		<div class="flex">
			...
			<div class="flex-initial">
				<a href="/article/{{ $article->id }}/edit">
					<button class="bg-green-500 text-white hover:bg-green-600 px-4 py-1">주문정보변경</button>
				</a>
				<form method="POST" action="/article/{{ $article->id }}" class="float-right ml-2">
				{{ method_field('delete') }}
				{{ csrf_field() }}
					<button class="bg-red-500 text-white hover:bg-red-600 px-2 py-1">주문취소</button>
				</form>
			</div>
		</div>
